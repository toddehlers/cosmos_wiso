...
USE mo_wiso,         ONLY : wiso_frac_liq_ice, tnat, twisoice, cwisomin, cwisosec
...
!---wiso-code
  IF (lwiso) THEN
     ...
     DO jt=1,kwiso
       DO jl=1,kproma
         IF(loflag(jl)) THEN
           zwisoqeen=pwisoqenh(jl,jk+1,jt)*(zdmfen(jl)+zoentr(jl,jk))
           zwisoqude=pwisoqu(jl,jk+1,jt)*zdmfde(jl)
           zdelta=tnat(jt)
           IF ((pwisoqenh(jl,jk+1,jt).GT.cwisomin).AND.(pqenh(jl,jk+1).GT.cwisomin)) zdelta=pwisoqenh(jl,jk+1,jt)/pqenh(jl,jk+1)
