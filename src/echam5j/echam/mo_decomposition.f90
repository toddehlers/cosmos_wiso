...
CONTAINS
  ...
  SUBROUTINE print_decomposition (dc)
    ...
    IF (ASSOCIATED(dc%glat)) WRITE (nout,'(a,12i5/(9x,12i5))') ' glat    : ', dc% glat
    IF (ASSOCIATED(dc%glon)) WRITE (nout,'(a,12i5/(9x,12i5))') ' glon    : ', dc% glon
    IF (dc%npts /= -1) THEN
       WRITE(nout,'(2(a,i5))') ' gptsse: ', dc%gptss, ' -', dc%gptse
    ENDIF
    WRITE (nout,*)
