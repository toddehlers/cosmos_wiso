...
!!    INTEGER           :: days, seconds
  ...
  SUBROUTINE init_events     !********* called in INITIALIZE *******************
    ...
    CALL echam_ev_init(ev_trigradm1,trigradm1,'albedo computation', EV_TLEV_PRES)
...
!
  ...
  REAL(dp) FUNCTION set_delta_time (truncation,nlevel)
    ...
    ELSE IF (nlevel == 90) THEN
      ...
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
    ELSE IF (nlevel == 91) THEN
      ! Middle atmosphere version
      IF (truncation == 511) THEN; set_delta_time =  60.0_dp
      ELSE
       CALL finish ('set_delta_time', 'Truncation not supported.')
      END IF
  ...
  FUNCTION str_date_native (no, date) RESULT (label)  !*************************
    ...
    SELECT CASE(my_no)
    CASE(FLT_YM)
      WRITE(label,'(i0.4,i2.2)')               yr,mo
    CASE(FLT_YMD)
      WRITE(label,'(i0.4,i2.2,a1,i2.2)')       yr,mo,'.',dy
    CASE(FLT_YMDH)
      WRITE(label,'(i0.4,i2.2,2(a1,i2.2))')    yr,mo,'.',dy,'_',hr
    CASE(FLT_YMDHM)
      WRITE(label,'(i0.4,i2.2,3(a1,i2.2))')    yr,mo,'.',dy,'_',hr,':',mn
    CASE(FLT_YMDHMS)
      WRITE(label,'(i0.4,i2.2,4(a1,i2.2))')    yr,mo,'.',dy,'_',hr,':',mn,':',se
    CASE default
      WRITE(label,'(i0.4,i2.2,a1,i2.2)')       yr,mo,'.',dy
    END SELECT
