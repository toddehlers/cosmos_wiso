...
!------------------------------------------------------------------------------
  ...
  USE mo_linked_list,  ONLY: memory_info, memory_type, t_stream, list_element,&
                             ...
                             HYBRID, HYBRID_H, TILES, SOILLEV, UNKNOWN,       &
                             GAUSSIAN, FOURIER, SPECTRAL, HEXAGONAL, GRIDPOINT, LAND
  ...
  USE mo_jsbach_comm_to_echam5mods,  ONLY: nland, nlon, nlat, &
                             domain_nland, domain_nlon, domain_nlat
...
!------------------------------------------------------------------------------
  ...
  PUBLIC :: free_unit_number       ! returns unused unit number
!------------------------------------------------------------------------------
  !
  ! Interfaces
  !
...
!-----------------------------------------------------------------------------
  ...
  CHARACTER (*)    ,INTENT(in) ,OPTIONAL :: dimnames(1)   ! dimension names
  LOGICAL          ,INTENT(in) ,OPTIONAL :: no_default    ! use no defaults
...
!==============================================================================
  ! --- free_unit_number() ------------------------------------------------------------------------------------------------------
  !
  ! Returns a unit number that is currently not used
  !
  integer function free_unit_number()
    ...
    do unit_no = 8,999
       unitOpened = .false.
       inquire(UNIT=unit_no,OPENED=unitOpened)
       if(.not. unitOpened) then
          free_unit_number = unit_no
          return
       end if
    end do
    call finish("free_unit_number()","No free unit found.")
  end function free_unit_number
    ! --- end free_unit_number() --------------------------------------------------------------------------------------------------
