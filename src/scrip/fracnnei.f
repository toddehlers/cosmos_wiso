...
C         are all masked points. Use of 4 non-masked nearest neighbours.
C
      nb_Vmm = 0
      ntottotland = 0
      beg_links = 1
...
C* -- Find the number of associated src points to the non-masked tgt point
                nb_links = 1
                DO WHILE (dst_addr(beg_links) .eq. 
     $              dst_addr(beg_links+nb_links))
                  nb_links = nb_links + 1
                END DO
...
C* -- If all the src points are land, treat it !
                IF (ntotland .EQ. ntotngh) THEN
                    nb_Vmm = nb_Vmm + 1
                    WRITE(nulou,*) 'Doing FRACNNEI for point', ib_dst  
...
C* -- Find the nearest neighbours and change weights and address 
                    ...
                    DO ib_src = 1, src_size
                      IF (ld_srcmask(ib_src)) THEN
                          distance = acos(coslat*cos(src_lat(ib_src))*
