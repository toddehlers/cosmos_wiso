...
!       env_name = toupper(TRIM(yname)) // '_THREADS'
       env_name = 'MPIOM_THREADS'

#ifdef NAG    
       CALL getenv('OMP_NUM_THREADS', thread_num, errno=p_error)
       IF (p_error /= 0) THEN
#else
       CALL getenv('OMP_NUM_THREADS', thread_num)
       IF (thread_num /= ' ') THEN
#endif
          WRITE (nerr,'(a)') ' Usage of OMP_NUM_THREADS disabled.'
          WRITE (nerr,'(a,a,a)') ' Please use ', TRIM(env_name), ' instead.' 
!          CALL p_abort
       END IF
